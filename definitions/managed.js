...
ctx =>`
    ...
    FROM 
        ${ctx.ref('TL_ext_clientes2')} cl
    JOIN 
        ${ctx.ref('TL_ext_enderecos2')} ad
        ON cl.cliente_cpf = ad.cliente_cpf
    JOIN 
        ${ctx.ref('TL_ext_itens_produtos_pedidos2')} it
        ON cl.cliente_cpf = it.pedido_cliente_cpf
    `)


transformation.transform(
    'TL_tb_itens_pedidos_clientes2', 
    'managed', 
    'table', 
    ['managed'], 
    ['TL_ext_clientes2', 'TL_ext_enderecos2', 
    'TL_ext_itens_produtos_pedidos2'], 
    query
)
